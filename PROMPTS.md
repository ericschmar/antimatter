...
For a coding agent:
  • Use prompts 3–7 one phase at a time.
  • Ask it to stop after verification rather than continuing into the     next phase.


For a review agent:
  • Use prompt 1 before implementation.
  • Use prompt 8 after implementation.


For a documentation agent:
  • Use prompt 9 only after code exists.


For a smaller/faster model:
  • Give only one phase prompt at a time.
  • Add the exact guide line range or section heading to reduce drift.
