...
describe("loadChannelHistoryWaterfall", () => {
	test("loads posts and authors without blocking on members", async () => {
		const log: CallLog[] = [];
		const api = fakeApi({
			log,
			posts: {
		...
		expect(data.memberUsers).toHaveLength(0);
		expect(data.postUsers.map((u) => u.id)).toEqual(["u1", "u2"]);
		expect(log.map((entry) => entry.method)).toEqual(["posts", "users"]);

		const members = await loadChannelMembersWaterfall(api, "ch1", "me", resolver);
		expect(members.members).toHaveLength(2);
		expect(members.memberUsers.map((u) => u.id)).toEqual(["u1", "u3"]);
	});
