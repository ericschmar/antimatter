...
describe("wrapSelection", () => {
	...
	test("supports multiline code-block markers", () => {
		const result = wrapSelection("code", { start: 0, end: 4 }, "\n```\n", "\n```\n");
...
describe("toggleLinePrefix", () => {
	test("adds the prefix to the current line", () => {
		expect(toggleLinePrefix("hello world", { start: 6, end: 6 }, "- ").message).toBe(
			"- hello world",
		);
	});

	test("removes an existing prefix (toggle off)", () => {
		expect(toggleLinePrefix("- hello", { start: 4, end: 4 }, "- ").message).toBe("hello");
	});

	test("only affects the line the caret is on", () => {
		expect(
			toggleLinePrefix("first\nsecond\nthird", { start: 7, end: 7 }, "> ").message,
		).toBe("first\n> second\nthird");
	});

	test("works on the first line", () => {
		expect(toggleLinePrefix("hello", { start: 0, end: 0 }, "# ").message).toBe("# hello");
	});
});
