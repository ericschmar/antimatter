		...
		<DropdownMenu.Root>
			<DropdownMenu.Trigger className="user-trigger">
				<span className="user-trigger-avatar">
					{imageSrc ? <img alt="" src={imageSrc} /> : initials(user.nickname || user.username)}
			...
			<DropdownMenu.Portal>
				<DropdownMenu.Content className="dropdown-content" sideOffset={8}>
					<DropdownMenu.Label className="dropdown-label">{user.username}</DropdownMenu.Label>
					<DropdownMenu.Item className="dropdown-item">
						{wsStatus === "connected" ? <Wifi size={14} /> : <WifiOff size={14} />}
						{wsStatus}
					</DropdownMenu.Item>
					<DropdownMenu.Separator className="dropdown-separator" />
					<DropdownMenu.Item className="dropdown-item danger" onSelect={onSignOut}>
						<LogOut size={14} />
						Sign out
					</DropdownMenu.Item>
