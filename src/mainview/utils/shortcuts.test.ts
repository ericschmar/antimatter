...
describe("readShortcutAction", () => {
	test("maps command search and settings shortcuts", () => {
		expect(readShortcutAction({ key: "k", metaKey: true })).toBe(
			"command-menu",
		);
	...
	test("maps section jump shortcuts", () => {
		expect(readShortcutAction({ key: "1", metaKey: true })).toBe(
			"navigate-favorites",
		);
		expect(readShortcutAction({ key: "2", metaKey: true })).toBe(
			"navigate-channels",
		);
		expect(readShortcutAction({ key: "3", metaKey: true })).toBe(
			"navigate-dms",
		);
	});

	test("maps channel, unread, and mention cycling shortcuts", () => {
		expect(readShortcutAction({ key: "[", metaKey: true })).toBe(
		...
		expect(
			readShortcutAction({ key: "{", metaKey: true, shiftKey: true }),
		).toBe("navigate-prev-unread");
		expect(
			readShortcutAction({ key: "}", metaKey: true, shiftKey: true }),
		).toBe("navigate-next-unread");
		expect(readShortcutAction({ key: "[", altKey: true, metaKey: true })).toBe(
	...
	test("maps composer shortcuts", () => {
		...
		expect(
			readShortcutAction({ key: "U", metaKey: true, shiftKey: true }),
		).toBe("attach-image");
