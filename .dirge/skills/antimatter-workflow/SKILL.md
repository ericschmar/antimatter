...
- The native app is a Swift Package for macOS 14+ (`Package.swift`): `Sources/AntimatterFoundation` is the shared Mattermost domain/API layer and `Sources/AntimatterApp` is the executable SwiftUI app. Foundation tests live in `NativeTests/AntimatterFoundationTests`; run `swift build && swift test` and `git diff --check` to verify native UI changes. The current suite has 29 passing tests, but it does not exercise App-target views.
