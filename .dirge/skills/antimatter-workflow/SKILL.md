...
- Nibware MCP search is category-sensitive: broad terms such as “message conversation” and “sidebar navigation” may return no chat components, while `Navigation` → “sidebar navigation” finds `nav2-sidebar`, and `Empty States` → “empty state” finds `es-search` and `es-messages`. Use these only as structural references and adapt their mobile-oriented rounded cards to Antimatter’s compact macOS dark surfaces.
...
## Native SwiftUI workspace UI

- The native app is a macOS 14+ Swift Package plus an Xcode app target: `Sources/AntimatterFoundation` is the shared Mattermost domain/API layer, `Sources/AntimatterApp` is the executable SwiftUI app, and `Antimatter.xcodeproj` produces `Antimatter.app`. Foundation tests live in `NativeTests/AntimatterFoundationTests`; run `swift build && swift test` and `git diff --check` to verify native UI changes. Build a local distributable app with `xcodebuild -project Antimatter.xcodeproj -scheme Antimatter -configuration Release -derivedDataPath build build`; its bundle is `build/Build/Products/Release/Antimatter.app`. Developer-ID distribution requires selecting a signing team because `DEVELOPMENT_TEAM` is presently empty.
- `README.md` and `.github/workflows/release.yml` are stale legacy ElectroBun/Bun documentation and packaging automation; do not rely on their Bun commands for the native SwiftUI app. A tag-release workflow for Xcode archive, signing, notarization, and artifact publishing still needs to be implemented.
- SwiftUI workspace composition is centered in `Sources/AntimatterApp/WorkspaceShell.swift`; feature views and their `@MainActor ObservableObject` view models are normally co-located as separate files (e.g. `SearchPanel`/`SearchViewModel`, `MessageComposer`/`ComposerViewModel`, `MessageTimeline`/`TimelineViewModel`). Import `AntimatterFoundation` first when Mattermost types are used, then AppKit/Foundation as needed, then SwiftUI.
- Preserve the dense graphite desktop-Mattermost visual language. Use `WorkspaceTheme` semantic tokens rather than literal colors; `navigationAccent` is for selection/navigation, `accent` for successful/presence actions, and `attention` for warnings or destructive actions. Prefer compact rounded rectangles and thin `WorkspaceTheme.divider` borders over card-heavy layouts.
- Only expose controls that have an implemented behavior. In particular, do not add cosmetic conversation-header action buttons (search, pins, files, info) until they have backing actions. Account-menu Settings should close its popover and call the existing `WorkspaceShell` settings presentation callback.
- Search completion needs an explicit state separate from `posts`: `SearchViewModel.hasSearched` distinguishes a finished zero-result search from untouched/empty input. Search result metadata can resolve channel display names from `MattermostChannel` and authors from the `[String: MattermostUser]` map; post timestamps are millisecond Unix epoch values.
- `ComposerViewModel` owns reply state: retain both `replyRootID` for the send request and `replyPost` for the visual reply preview, and clear both after send or cancellation.
- Team and profile controls are nested under `LargeTitleHeader` in `WorkspaceShell`, while `isSettingsPresented` belongs to the outer `WorkspaceShell`. Thread an `onOpenSettings` closure through intermediary views rather than capturing that outer `@State` from a child scope.
- The source editor's Swift syntax guard may reject an unbalanced multi-line SwiftUI builder update. When it does, read the exact affected range, replace the full builder body with balanced structure, then run `swift build`; an earlier attempted piecemeal rewrite of `ChannelParticipantStack` failed this guard because of mismatched braces.
- The generic todo-board sync can report `database disk image is malformed`; this did not affect source changes or Beads tracking. Do not use it for Antimatter task tracking—use `bd`.
