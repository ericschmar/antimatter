...
struct ProfileSettingsForm: View {
    ...
    var body: some View {
        SettingsGroup {
            ...
            SettingsDivider()
            SecureProfileField("Current password", text: $model.currentPassword)
            SettingsDivider()
            SecureProfileField("New password", text: $model.newPassword)
            HStack {
                Spacer()
                Button("Change password") { Task { await model.changePassword() } }
                    .buttonStyle(.bordered)
                    .disabled(model.currentPassword.isEmpty || model.newPassword.isEmpty || model.isSaving)
            }
            .padding(.horizontal, 16)
            .padding(.top, 12)
            HStack {
